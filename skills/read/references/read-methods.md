...
marker_single /path/to/file.pdf --output_dir "${READ_OUTPUT_DIR:-/tmp/read-out}"
...
if [ -z "$READ_SCRIPT_DIR" ]; then
  echo "read helper scripts not found; set CLAUDE_SKILL_DIR or run from the skills repo root" >&2
  exit 1
fi
```
