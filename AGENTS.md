...
- 正文中文为主，默认结构：`# 名: 一句签名式本质` → 2-4 句本质 + 要避免的失败模式 → `## Outcome Contract` → `## Core Stance` →（可选）`## Pre-flight` →（可选）`## 模式`（每个 `**Activate when**: ...`）→ `## Hard Rules` → `## Gotchas`（表格）→（可选）`## Closing Pass` → `## Output`。`check` / `health` 这类复杂调度 skill 可用等价专用章节，但新增 skill 默认按标准结构写。
